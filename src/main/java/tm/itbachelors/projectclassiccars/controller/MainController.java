...
public class MainController {
    ...
    public String createOwner(@RequestParam String firstName, @RequestParam String surName, @RequestParam(defaultValue = "0") int yearOfBirth, @RequestParam(defaultValue = "-1") int carIndex, Model model) {
        Owner owner = new Owner(firstName, surName);
        owner.setYearOfBirth(yearOfBirth);
        if (carIndex >= 0) {
            carArrayList.get(carIndex).registerOwner(owner);
        }
        ownerArrayList.add(owner);
        model.addAttribute("owner", owner);
        return "2_owner_details";
    }

    @GetMapping("/newStaff")
    public String newStaff() {return "3_new_staff";}

    @PostMapping("/createStaff")
    public String createStaff(@RequestParam String firstName, @RequestParam String surName, @RequestParam String startDate, @RequestParam(defaultValue = "false") boolean senior, Model model) {
        Staff staff = new Staff(firstName, surName);
        DateTimeFormatter dtf = DateTimeFormatter.ofPattern("dd/MM/yyyy");
        staff.setStartDate(LocalDate.parse(startDate, dtf));
        staff.setSenior(senior);
        staffArrayList.add(staff);
        model.addAttribute("staff", staff);
        return "4_staff_details";
    }
    ...
    public String createCar(@RequestParam String brand,
                            ...
                            @RequestParam(defaultValue = "") String photo,
                            Model model) {
    ...
    public String newMaintenance(Model model) {
        model.addAttribute("cars", carArrayList);
        model.addAttribute("staffList", staffArrayList);
        return "9_new_maintenance";
    }

    @PostMapping("/createMaintenance")
    public String createMaintenance(@RequestParam String type, @RequestParam int duration, @RequestParam double hourlyRate, @RequestParam int carIndex, @RequestParam int staffIndex, Model model) {
        if (carIndex < 0) {
            model.addAttribute("errorMessage", "You didn't choose a car!");
            return "error";
        }
        if (staffIndex < 0) {
        ...
        maintenance.setResponsible(staffArrayList.get(staffIndex));
        carArrayList.get(carIndex).addMaintenance(maintenance);
        model.addAttribute("car", carArrayList.get(carIndex));
        return "10_maintenance_list";
    }

    @GetMapping("/maintenanceList")
    public String maintenanceList(@RequestParam int carIndex, Model model) {
        model.addAttribute("car", carArrayList.get(carIndex));
        return "10_maintenance_list";
    }

    @PostMapping("/searchMaintenance")
    public String searchMaintenance(@RequestParam String searchType, Model model) {
        ArrayList<Maintenance> results = new ArrayList<>();
        for (Car car : carArrayList) {
            Maintenance found = car.searchMaintenanceByType(searchType);
            if (found != null) {
                results.add(found);
            }
        }
    ...
    private ArrayList<Owner> fillOwners() {
        ...
        Owner myOwner = new Owner("Talha", "Arif");
        myOwner.setYearOfBirth(2006);
        myOwner.addToWishList("Toyota - Supra");
        myOwner.addToWishList("Honda - NSX");
        ownerArrayList.add(myOwner);
